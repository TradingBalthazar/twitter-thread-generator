/**
 * Mock implementation of Vercel KV for local development
 * 
 * This provides a simple in-memory implementation of the Vercel KV API
 * that can be used for local development without requiring the Vercel CLI.
 * 
 * To use this, modify the imports in your code to use this implementation
 * instead of the real Vercel KV client.
 * 
 * Example:
 * ```
 * // Instead of:
 * import { kv } from '@vercel/kv';
 * 
 * // Use:
 * import { kv } from '@/lib/kv/mock';
 * ```
 */

// In-memory storage
const store = new Map<string, string>();

// Mock KV implementation
export const kv = {
  // Get a value
  get: async (key: string): Promise<string | null> => {
    console.log(`[Mock KV] GET ${key}`);
    return store.get(key) || null;
  },
  
  // Set a value
  set: async (key: string, value: any): Promise<void> => {
    console.log(`[Mock KV] SET ${key}`);
    store.set(key, typeof value === 'string' ? value : JSON.stringify(value));
  },
  
  // Delete a value
  del: async (key: string): Promise<void> => {
    console.log(`[Mock KV] DEL ${key}`);
    store.delete(key);
  },
  
  // Get all keys matching a pattern
  keys: async (pattern: string): Promise<string[]> => {
    console.log(`[Mock KV] KEYS ${pattern}`);
    const regex = new RegExp(pattern.replace('*', '.*'));
    return Array.from(store.keys()).filter(key => regex.test(key));
  },
  
  // Check if a key exists
  exists: async (key: string): Promise<number> => {
    console.log(`[Mock KV] EXISTS ${key}`);
    return store.has(key) ? 1 : 0;
  },
  
  // Get multiple values
  mget: async (...keys: string[]): Promise<(string | null)[]> => {
    console.log(`[Mock KV] MGET ${keys.join(', ')}`);
    return keys.map(key => store.get(key) || null);
  },
  
  // Set multiple values
  mset: async (...keyValues: (string | number)[]): Promise<void> => {
    console.log(`[Mock KV] MSET ${keyValues.join(', ')}`);
    for (let i = 0; i < keyValues.length; i += 2) {
      const key = keyValues[i].toString();
      const value = keyValues[i + 1];
      store.set(key, typeof value === 'string' ? value : JSON.stringify(value));
    }
  },
  
  // Increment a value
  incr: async (key: string): Promise<number> => {
    console.log(`[Mock KV] INCR ${key}`);
    const value = store.get(key);
    const newValue = value ? parseInt(value, 10) + 1 : 1;
    store.set(key, newValue.toString());
    return newValue;
  },
  
  // Expire a key
  expire: async (key: string, seconds: number): Promise<number> => {
    console.log(`[Mock KV] EXPIRE ${key} ${seconds}`);
    if (!store.has(key)) return 0;
    
    // In this mock implementation, we don't actually expire keys
    // In a real implementation, you would set a timeout to delete the key
    return 1;
  },
};